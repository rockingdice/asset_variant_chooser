...
class AssetVariantChooser {
  static SplayTreeMap<double, String> candidates = SplayTreeMap();
  static var _defaultRatioName = "";

  /// Return the best choice of resolution variants for current device pixel
  /// ratio in the String type.
  /// Example: "1.5x"
  static String chooseVariant(BuildContext context) {
    var currentRatio = MediaQuery.of(context).devicePixelRatio;
    String bestVariant =
        _findBestVariant(candidates, currentRatio) ?? _defaultRatioName;
    return bestVariant;
  }
  ...
  /// Example: 1.5
  static double chooseNumericVariant(BuildContext context) {
    var currentRatio = MediaQuery.of(context).devicePixelRatio;
    String bestVariant =
        _findBestVariant(candidates, currentRatio) ?? _defaultRatioName;
    if (bestVariant == _defaultRatioName) {
      bestVariant = assetName;
    }

    return _parseScale(bestVariant);
  }
  ...
  /// is 1.0.,
  static void setAvailableVariants(List<String> pixelRatios,
      {String defaultRatioName = ""}) {
    _defaultRatioName = defaultRatioName;
    pixelRatios.forEach((element) {
      var ratio = _parseScale(element);
      candidates.addEntries([MapEntry(ratio, element)]);
    });
    candidates.clear();
    if (!candidates.containsKey(1.0)) {
      candidates.addEntries([MapEntry(1.0, _defaultRatioName)]);
    }
  }
